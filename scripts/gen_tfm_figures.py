...
ax.bar(["StratifiedKFold\n(by event, with leakage)", "GroupKFold\n(by user, honest)"],
       [0.695, 0.563], color=[GRIS, ROJO], edgecolor="white")
ax.axhline(0.5, color=NEUTRO, ls="--", lw=1, label="chance (0,5)")
for x, v in zip([0, 1], [0.695, 0.563]):
    ax.text(x, v + 0.01, f"{v:.3f}".replace(".", ","), ha="center", fontweight="bold")
ax.set_ylabel("ROC-AUC (OOF)"); ax.set_ylim(0, 0.85); ax.legend()
ax.set_title("M2: effect of correcting data leakage")
...
ax.axhline(0.238, color=NEUTRO, ls="--", lw=1, label="chance (prevalence 0,238)")
for x, v in enumerate(pr):
    ax.text(x, hi[x] + 0.004, f"{v:.3f}".replace(".", ","), ha="center", fontsize=9)
ax.set_ylabel("PR-AUC (95\% CI)"); ax.set_ylim(0, 0.45); ax.legend()
ax.set_title("M2: PR-AUC by strategy (GroupKFold by user)")
...
ax.set_xlabel("Importance (no. of splits, LightGBM)")
ax.set_title("M2: feature importance (model with product)")
...
datos = {"Popularity": [0.629, 0.806, 0.580, 0.500],
         "Demographic cluster": [0.629, 0.789, 0.572, 0.496],
         "Per-cluster recommender (M1)": [0.586, 0.774, 0.521, 0.439]}
...
ax.legend(fontsize=8); ax.set_title("M3: per-cluster recommender (M1) vs baselines (test, no leakage)")
...
axes[0].plot(ks, sil, "o-", color=AZUL); axes[0].axvline(10, color=ROJO, ls="-", label="k=10 (adopted)")
axes[0].set_title("Mean silhouette"); axes[0].set_xlabel("k"); axes[0].legend(fontsize=8)
axes[1].plot(ks, ari, "o-", color=NARANJA); axes[1].axhline(0.9, color=NEUTRO, ls="--", lw=1, label="ARI=0,90")
axes[1].axvline(10, color=ROJO, ls="-", label="k=10 (adopted)")
axes[1].set_title("Stability (ARI across resamples)"); axes[1].set_xlabel("k"); axes[1].legend(fontsize=8)
...
modelos_m4 = ["Prophet", "Naïve", "Moving average 3"]
...
ax.set_ylabel("MAE (clicks/week)"); ax.set_ylim(0, 850)
ax.set_title("M4: 1-step error (rolling backtest) — lower is better")
...
ax.bar(x - w/2, roi_bd, w, label="DB (send to all)", color=GRIS, edgecolor="white")
ax.bar(x + w/2, roi_modelo, w, label="Model (prioritise)", color=ROJO, edgecolor="white")
...
ax.set_xticks(x); ax.set_xticklabels(escenarios); ax.set_ylabel("Aggregate ROI (\\%)")
ax.set_title("ROI by per-email cost structure: DB vs model"); ax.legend()

...
barras = ax.bar(["Open", "Click"], [vc.get("open", 0), vc.get("click", 0)],
...
axes[0, 0].set_title("Age (continuous)"); axes[0, 0].set_xlabel("years"); axes[0, 0].set_ylabel("users")
...
axes[0, 1].set_title("Categorised age (age_cat, M2)"); axes[0, 1].set_ylabel("users")
axes[0, 1].tick_params(axis="x", labelrotation=30)

# Género
g = users["gender"].value_counts()
axes[0, 2].bar([{"H": "Male", "M": "Female"}.get(k, k) for k in g.index], g.values,
               color=AZUL, edgecolor="white", width=0.6)
axes[0, 2].set_title("Gender"); axes[0, 2].set_ylabel("users")

# Situación laboral
lab = users["labor_status"].value_counts()
lab_lbl = {"employed": "Employed", "inactive": "Inactive", "unemployed": "Unemployed"}
axes[1, 0].bar([lab_lbl.get(k, k) for k in lab.index], lab.values,
               color=AZUL, edgecolor="white", width=0.6)
axes[1, 0].set_title("Labour status (synthetic)"); axes[1, 0].set_ylabel("users")
axes[1, 0].tick_params(axis="x", labelrotation=20)

# Estado civil
civ = users["civil_status"].value_counts()
civ_lbl = {"casado": "Married", "soltero": "Single", "divorciado": "Divorced", "viudo": "Widowed"}
axes[1, 1].bar([civ_lbl.get(k, k) for k in civ.index], civ.values,
               color=AZUL, edgecolor="white", width=0.6)
axes[1, 1].set_title("Marital status (synthetic)"); axes[1, 1].set_ylabel("users")
axes[1, 1].tick_params(axis="x", labelrotation=20)

# Clase de renta (IPA)
ipa = users["ipa_class"].value_counts().sort_index()
ipa_lbl = {0: "Low", 1: "Low-mid", 2: "Mid", 3: "Mid-high", 4: "High"}
axes[1, 2].bar([ipa_lbl.get(k, k) for k in ipa.index], ipa.values,
               color=AZUL, edgecolor="white", width=0.6)
axes[1, 2].set_title("Income class (IPA, geographic)"); axes[1, 2].set_ylabel("users")
axes[1, 2].tick_params(axis="x", labelrotation=20)

fig.suptitle("User demographic distributions", fontsize=13)
...
ax.axvline(0.238, color=NEUTRO, ls="--", lw=1, label="Global CTR (0,238)")
for i, (v, n) in enumerate(zip(ctr["ctr"], ctr["n"])):
    ax.text(v + 0.005, i, f"{v:.2f}  (n={n:,})", va="center", fontsize=8)
ax.set_xlabel("Click-through rate (clicks / events)")
ax.set_title("Click-through rate by sector")
...
ax.set_xlabel("No. of events per user"); ax.set_ylabel("users")
ax.set_title("Activity per user (user--product matrix 98,1\\% empty)")
...
axes[0].set_xlabel("No. of events"); axes[0].set_title("Top 12 categories by volume (colour: CTR vs mean)")
...
axes[1].axvline(cpl.median(), color=ROJO, ls="--", lw=1.2, label=f"median = {cpl.median():.0f}\\,\\euro")
axes[1].set_xlabel("CPL (\\euro\\ per lead)"); axes[1].set_ylabel("no. of products")
axes[1].set_title(f"Price distribution ({cpl.shape[0]}/{len(prods)} products)"); axes[1].legend()
...
axes[0].axhline(0.238, color=NEUTRO, ls="--", lw=1, label="Global CTR (0,238)")
axes[0].set_xlabel("Age (years)"); axes[0].set_ylabel("Click-through rate")
axes[0].set_title("CTR by continuous age (5-year bins)"); axes[0].legend()
...
axes[1].set_ylabel("Click-through rate"); axes[1].set_title("CTR by age band (age\\_cat, M2)")
axes[1].tick_params(axis="x", labelrotation=20)
# Derecha: género
axes[2].bar([{"H": "Male", "M": "Female"}.get(k, k) for k in ctr_gen.index], ctr_gen.values,
            color=ROJO, edgecolor="white", width=0.55)
axes[2].axhline(0.238, color=NEUTRO, ls="--", lw=1)
axes[2].set_ylabel("Click-through rate"); axes[2].set_title("CTR by gender")
fig.suptitle("Click-through rate by real demographic variables (continuous vs categorised age)", fontsize=12)
